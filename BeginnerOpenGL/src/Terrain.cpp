...
void Terrain::SetChunkSize(unsigned int x, unsigned int z)
{
	_xAmount = x;
	_zAmount = z;
	_chunkLength = glm::vec2((float)_terrainSize.x / (float)_xAmount, _terrainSize.y / (float)_zAmount);
	_chunks.clear();
	for (int i = 0; i < _xAmount; i++) {
		_chunks.push_back(std::vector<MarchingCubes>());
		for (int j = 0; j < _zAmount; j++)
		{
			_chunks[i].emplace_back("res/shaders/ctest2.comp", "res/shaders/marching.shader");
		}
	}
	UpdateChunks();
}
...
void Terrain::UpdateChunks()
{
	for (int i = 0; i < _xAmount; i++) {
		for (int j = 0; j < _zAmount; j++) {
			...
			//printf("x : z           : %f   :   %f", x, z);
			_chunks[i][j].UpdateTransform(glm::vec3(x, _pos.y, z), _chunkLength.x);
		}
	}
}
